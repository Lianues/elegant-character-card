...
ecc world-book init                         # 用内置「空世界书」模板初始化新仓库
ecc world-book init 我的新世界书             # 指定世界书 / 仓库名
...
## 世界书初始化（`ecc world-book init`）

`ecc world-book init [name]` 用来**从零开搭一个独立世界书仓库**，避免手写 `_metadata.yaml` / `entries/` 目录结构。

其本质等价于：从项目内置的「空卡」模板（`src/空卡.png`）中提取内嵌的 `character_book`（即「空世界书」），再调用 `world-book repo` 的仓库化流程。

- 种子源：`src/空卡.png` 内嵌的 `character_book`（编译后位于 `dist/空卡.png`，与 `ecc init` 共用模板）
- 包含 1 条占位 `entries`，可按需删改

用法：

```bash
ecc world-book init                          # 生成名为「空世界书」的仓库目录
ecc world-book init 我的新世界书              # 用「我的新世界书」覆盖 world_book.name，仓库目录同名
ecc world-book init 我的新世界书 -c custom.yaml  # 自定义配置文件路径（不存在时回落到内置默认）
```

生成产物（以 `ecc world-book init 我的新世界书` 为例）：

```
我的新世界书/
├── _metadata.yaml          # 世界书元数据（name/description/scan_depth/...）
└── entries/                # 世界书条目目录（每个 .yaml 一条 entry，按 path_chain 嵌套子目录）
    └── 1.yaml              # 内置 1 条占位条目
```

下一步即可编辑 `entries/` 下的条目文件，然后用 `ecc world-book build 我的新世界书` 产出最终 `world_book.json`；或者直接 `cd 我的新世界书 && ecc world-book build`。
