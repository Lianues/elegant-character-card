...
function encodeMetadataToBase64(metadata: string): string {
  return Buffer.from(metadata, "utf-8").toString("base64");
}

function decodeMetadataFromBase64(encoded: string): string {
  return Buffer.from(encoded, "base64").toString("utf-8");
}

function withoutCardMetadataChunks(chunks: PngChunk[]): PngChunk[] {
...
function insertBeforeIend(chunks: PngChunk[], extraChunks: PngChunk[]): PngChunk[] {
  const cloned = [...chunks];
 ...
 */
export async function readPngTextChunks(imagePath: string): Promise<PngTextChunk[]> {
 ...
 */
export async function embedCardMetadata(
  metadata: string,
  imagePath: string,
  outputPath: string,
  legacy = false,
 ...
 */
export async function extractCardMetadata(imagePath: string): Promise<string | null> {
  const textChunks = await readPngTextChunks(imagePath);

  const primaryChunk = textChunks.find(
    (chunk) => chunk.keyword === CARD_METADATA_PRIMARY_KEY,
  );
  ...
  const legacyChunk = textChunks.find(
    (chunk) => chunk.keyword === CARD_METADATA_LEGACY_KEY,
  );
 ...
 */
export async function stripCardMetadataFromPng(
  inputPath: string,
  outputPath: string,
): Promise<void> {
