...
} from "../models/cardSchemas.js";

/**
 * 严格按 V3（新格式）解析角色卡。
 */
export function parseCharacterCardV3(input: unknown): CharacterCardV3 {
 ...
 */
export function upgradeV2ToV3(cardV2: CharacterCardV2): CharacterCardV3 {
  const payload = {
    data: cardV2.data,
  };

  const normalized = normalizeLegacyBookInPayload(payload) as { data: unknown };
 ...
 */
export function parseV3OrUpgradeFromV2(input: unknown): {
